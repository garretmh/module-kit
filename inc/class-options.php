...
class MK_Options {
	 ...
	 * @var string
	 */
	protected $key = 'mk_options';

	/**
	 * Options page metabox id
	 * @var string
	 */
	protected $metabox_id = 'mk_option_metabox';

	/**
	 * Options Page title
	 * @var string
	 */
	protected $title = 'Module Kit';
	...
	function add_options_page_metabox() {
		...
		foreach( get_mk_modules() as $modules ) {
			$field = array(
				'name' => __( $modules->title, 'mk' ),
				'id'   =>     $modules->key,
				'type' => 'checkbox',
			);
			$cmb->add_field( $field );
 ...
 */
function mk_options() {
	return MK_Options::get_instance();
}
 ...
 */
function mk_get_option( $key = '' ) {
	return cmb2_get_option( mk_options()->key, $key );
}
